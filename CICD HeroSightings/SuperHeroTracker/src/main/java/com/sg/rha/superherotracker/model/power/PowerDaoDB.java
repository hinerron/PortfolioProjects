...
public class PowerDaoDB implements PowerDao {
    ...
    public Power getPowerById(int id) throws ElementNotFoundException {
        try {
            ...
            Power power = jdbc.queryForObject(GET_POWER_BY_ID, new PowerMapper(), id);
            power.setHeroes(getAllHeroesByPower(id));
            return power;
    ...
    @Override
    public List<Hero> getAllHeroesByPower(int id) {
        List<Hero> heroesByPower = new ArrayList<>();

        final String GET_ALL_HEROES_BY_POWER = "SELECT * FROM HeroPower hp "
                + "JOIN Hero h ON hp.heroId = h.heroId "
                + "WHERE powerId = ?";

        heroesByPower = jdbc.query(GET_ALL_HEROES_BY_POWER, new HeroDaoDB.HeroMapper(), id);
